...
- 🔴 **INTERRUPTED — SECURITY AUDIT (resume here):** codebase clean (`.env*` gitignored/untracked, no hardcoded secrets, bundle carries only the public anon/VAPID/Stripe-price values, `scan-props`/`scan-edges` gated by `requireAuth` + credit breaker). **Still to verify:** (1) run Supabase security advisor + confirm the 2 over-permissive INSERT policies (`odds_history` anon, `lean_results` auth `WITH CHECK(true)`) are tightened; (2) enable leaked-password protection (Auth dashboard, owner); (3) `api/cron-kbo-leans.js` has **no CRON_SECRET guard** (free scan, low risk, but add one).
